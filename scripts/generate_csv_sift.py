...
def main(argv):
    ...
    fieldnames = ["concept", "centers", "g", "w", "map"]
    writer = csv.DictWriter(fo, fieldnames=fieldnames)
    writer.writeheader()
    for result in results:
        fname = os.path.basename(result)
        values = re.split("_g-|_w-|centers-", fname)
        files = glob.glob(os.path.join(result, "sift_val/trec_eval_results") + "/*")
            ...
            with open(file_res, "rb") as trec_res:
                for line in trec_res.read().splitlines():
                    if "map" in line and "gm_map" not in line and concept_name in line:
                        ...
                        row[fieldnames[3]] = values[3]
                        row[fieldnames[4]] = token_line[-1]
                        writer.writerow(row)
    fo.close()
